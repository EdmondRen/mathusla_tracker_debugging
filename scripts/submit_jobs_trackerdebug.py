...
def main():
    ...
    parser.add_argument('--hours', type=int, default=4, help='Requested time in hours per job')
    ...
    for energy in EnergyList:
        for i, sim_script in enumerate(Scripts):
            if i==0:
                job_script=f"""mkdir -p {DataDir}/muon_{energy}_GeV_manual \n
{simulation} -j1 -q  -o {DataDir}/muon_{energy}_GeV_manual  -s {sim_script} energy {energy} count {EventCount}            
                """
            if i==1:
                job_script=f"""mkdir -p {DataDir}/pion_{energy}_GeV_manual \n
{simulation} -j1 -q  -o {DataDir}/pion_{energy}_GeV_manual  -s {sim_script} energy {energy} count {EventCount}            
                """ 
                
            # This is the core function to submit job script
            script_prefix=sim_script.split("_")[0]
            submit_script(job_script, f"{script_prefix}_{energy}", blockidx=0, hours=hours, log_dir=Log_Dir, job_name="reco", system=system, slurm_account=slurm_account, slurm_partition='', debug=DEBUG, verbose=verbose)
